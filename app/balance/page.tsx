...
function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      />
      <div
        className="relative z-10 w-full max-w-md rounded border border-[#1e2430] bg-[#12151c] p-5 shadow-xl"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="mb-4 flex items-start justify-between gap-3">
          <h2
            ...
            className="font-mono text-[12px] uppercase tracking-wider text-[#ffc107]"
          >
            {title}
          </h2>
          <button
            type="button"
            onClick={onClose}
            className="shrink-0 border border-[#2a3140] px-2.5 py-1 font-mono text-[13px] leading-none text-[#5c6578] hover:border-[#5c6578] hover:text-[#c8d0e0]"
            aria-label="Close"
          >
            ×
          </button>
  ...
  return (
    <div className="min-h-screen bg-[#0c0e12] text-[#c8d0e0]">
      <div className="mx-auto max-w-3xl px-4 py-8">
        ...
        {showSignIn && (
          <p className="mb-6 rounded border border-[#2a3140] bg-[#12151c] p-4 text-[11px] text-[#5c6578]">
        ...
        >
          <button
            ...
            className="rounded border-2 border-[#00e5ff]/40 bg-[#0a1620] px-5 py-6 text-center font-mono text-[14px] uppercase tracking-wide text-[#00e5ff] transition hover:border-[#00e5ff] hover:bg-[#0c1a24] disabled:cursor-not-allowed disabled:opacity-40"
          >
            Top up
          </button>
          <button
            ...
            className="rounded border-2 border-[#ff5252]/40 bg-[#1a0c0c] px-5 py-6 text-center font-mono text-[14px] uppercase tracking-wide text-[#ff8a80] transition hover:border-[#ff5252] hover:bg-[#221010] disabled:cursor-not-allowed disabled:opacity-40"
          >
            Withdraw
          </button>
        </div>
          ...
          {topUpLoadingWallet ? (
            <div className="flex flex-col items-center gap-4 py-6 text-center">
              <div
                ...
                aria-hidden
              />
              <p className="text-[11px] text-[#5c6578]">Loading wallet…</p>
            </div>
          ) : showNoDeposit ? (
            <div className="flex flex-col items-center gap-5 py-2 text-center">
              ...
              />
              <div>
                <p className="mb-2 font-mono text-[12px] text-[#c8d0e0]">
            ...
            <div className="flex flex-col items-center gap-4 sm:flex-row sm:items-start">
              <div className="rounded bg-white p-4">
                {qrSrc ? (
                  ...
                  <div className="flex h-[200px] w-[200px] items-center justify-center bg-[#eee] text-[11px] text-[#666]">
                    —
                  </div>
                )}
              </div>
              <div className="min-w-0 flex-1">
                ...
                <div className="mb-2 break-all font-mono text-[12px] leading-relaxed text-[#e0e0e0]">
                  {depositAddress || "—"}
                </div>
                <button
                  type="button"
                  ...
                  className="min-h-[2.5rem] border border-[#2a3140] px-4 py-2 text-[11px] font-mono uppercase text-[#00e5ff] hover:border-[#00e5ff] disabled:opacity-40"
                >
                  {copied ? "Copied" : "Copy address"}
                </button>
                <p className="mt-3 text-[11px] leading-relaxed text-[#5c6578]">
        ...
        >
          <p className="mb-4 text-[11px] leading-relaxed text-[#5c6578]">
          ...
          <div className="mb-3 flex items-baseline justify-between gap-2">
            <span className="text-[11px] uppercase tracking-wider text-[#5c6578]">
              Available to withdraw
            </span>
            <span className="font-mono text-[13px] text-[#c8d0e0]">
              {!user || !hydrated ? "—" : formatUsd(balance)}
            </span>
          </div>
          <div className="flex flex-col gap-3">
            <input
              ...
              className="min-h-[2.75rem] w-full border border-[#2a3140] bg-[#0c0e12] px-3 py-2.5 font-mono text-[11px] text-[#c8d0e0] outline-none focus:border-[#ffc107]"
              value={withdrawDest}
            ...
            <div className="flex flex-wrap items-stretch gap-2">
              ...
              <button
                ...
                className="min-h-[2.75rem] shrink-0 border border-[#2a3140] px-4 py-2.5 text-[11px] font-mono uppercase text-[#ffc107] hover:border-[#ffc107] disabled:opacity-40"
              >
                Max
              </button>
            </div>
            <button
              ...
              disabled={!user || withdrawBusy}
              className="min-h-[2.75rem] border border-[#ff5252]/60 bg-[#1a0c0c] py-3 text-[12px] font-mono uppercase text-[#ff8a80] hover:border-[#ff5252] disabled:opacity-40"
            >
              {withdrawBusy ? "Submitting…" : "Confirm withdrawal"}
            </button>
          </div>
          {withdrawError && (
            <p className="mt-3 text-[11px] text-[#ff5252]">{withdrawError}</p>
          )}
        </Modal>
        ...
        >
          ...
          <div className="max-h-[380px] overflow-y-auto">
            <div className="grid grid-cols-[1fr_1fr_1fr_auto] gap-x-3 border-b border-[#1e2430] px-4 py-2 font-mono text-[11px] text-[#5c6578]">
              <span>TIME</span>
              <span>KIND</span>
            ...
            {ledgerRows.map((row) => (
              <div
                key={row.id}
                className="grid grid-cols-[1fr_1fr_1fr_auto] gap-x-3 border-b border-[#1a1f28] px-4 py-2 font-mono text-[11px]"
              >
